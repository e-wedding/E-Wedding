...
function doPost(e) {
  ...
  try {
    ...
    // USE e.parameter instead of JSON.parse to avoid CORS preflight failures
    var data = e.parameter;

    // Append the data as a new row (Keys must match HTML 'name' attributes)
    sheet.appendRow([
      new Date(),                 // Timestamp
      data.theme || "N/A",        // Selected theme
      data.package || "Premium",  // Selected package
      data.coupleNames || "N/A",  // Couple's names (Matches name="coupleNames")
      data.weddingDate || "N/A",  // Wedding date (Matches name="weddingDate")
      data.venue || "N/A",        // Venue/location (Matches name="venue")
      data.email || ""            // Email 
    ]);
...
function sendAutoReplyEmail(data) {
    ...
    <div style="font-family: 'Georgia', serif; padding: 30px; background: #f8f9fa; border-radius: 10px;">
      <div style="max-width: 500px; margin: 0 auto; background: white; padding: 30px; border-radius: 10px; box-shadow: 0 4px 15px rgba(0,0,0,0.1);">
        ...
        <p style="color: #555; font-size: 14px; line-height: 1.8; margin-bottom: 20px;">
          ...
          <strong style="color: #D4AF37;">${data.package || 'Premium'}</strong> package for your celebration at
        ...
        <div style="background: #FDF5E6; padding: 20px; border-radius: 8px; margin: 25px 0;">
          ...
          <ul style="color: #555; font-size: 13px; line-height: 2; margin: 0; padding-left: 20px;">
            ...
            <li>Review and customize your ${data.package || 'Premium'} package features</li>
  ...
  var plainTextBody = `Dear ${data.coupleNames},\n\n` +
    `Thank you for choosing our ${data.theme} theme with the ${data.package || 'Premium'} package for your celebration at ${data.venue} on ${data.weddingDate}.\n\n` +
    `Our team is reviewing your details and will reach out via WhatsApp shortly to finalize your custom e-invite design.\n\n` +
    `What's Next:\n` +
    `- Our designer will contact you within 24 hours\n` +
    `- Review and customize your ${data.package || 'Premium'} package features\n` +
