...
|-------|-------|----------|
| 🏖️ Magnificent Beach | Coastal, Modern | Batu Tinagat, Tawau beaches |
| 🌿 Enchanted Rainforest | Lush, Intimate | Rainforest venues |
| 🏨 Luxury Resort | Elegant, Premium | Borneo Royale Hotel |
| 🌸 Garden & Outdoor | Rustic, Casual | Cowie Bay, outdoor venues |
| ⛪ Classical Chapel | Timeless, Traditional | Chapel/indoor weddings |
...
```bash
# Push to GitHub, then connect to Vercel/Netlify
# Or use CLI:
vercel deploy
# or
netlify deploy
...
```javascript
const WHATSAPP_NUMBER = "60123456789"; // Your Malaysia number with country code
```
...
```javascript
// Remove the /* and */ comments around this section:
if (data.email) {
...
├── style.css               # All styling (Bento Grid, themes, responsive)
├── script.js               # Frontend logic (modal, forms, integrations)
...
```css
:root {
    --beach-primary: #00B4D8;      /* Beach theme color */
    --rainforest-green: #2D6A4F;   /* Rainforest theme color */
    --luxury-gold: #D4AF37;        /* Luxury theme accent */
    --garden-sage: #9CAF88;        /* Garden theme color */
}
```
...
3. Test manually: `https://wa.me/60123456789`
...
```
User selects theme → Modal opens → User fills form
                                         ↓
                    ┌────────────────────┴────────────────────┐
                    ↓                                         ↓
            WhatsApp Button                           Google Sheets Button
                    ↓                                         ↓
    Pre-filled message sent                        Data saved to Sheet
    to your WhatsApp                               (Optional: Auto-email)
```
